...
import type { CoreColor, GameStateSnapshot, LevelCompleteData } from './types.js';

type ConduitClickCallback = (index: number) => void;
type NextLevelCallback = () => void;

export class Renderer {
  private container: HTMLElement | null;
  private selectedConduit: number | null = null;
  private onConduitClick: ConduitClickCallback | null = null;
  private onNextLevel: NextLevelCallback | null = null;
  ...
  setSelectedConduit(index: number | null): void {
  ...
  animateCoreTransfer(fromIndex: number, toIndex: number, color: CoreColor, onComplete: () => void): void {
    if (!this.container) {
      onComplete();
      return;
    }

    const conduits = Array.from(this.container.querySelectorAll<HTMLElement>('.conduit'));
  ...
  onConduitClickCallback(callback: ConduitClickCallback): void {
    this.onConduitClick = callback;
  }

  onNextLevelCallback(callback: NextLevelCallback): void {
    this.onNextLevel = callback;
  }

  updateControls(state: GameStateSnapshot): void {
    ...
    if (levelDisplay) levelDisplay.textContent = `Level ${state.levelId}`;
  }
  ...
  showLevelComplete(data: LevelCompleteData): void {
    ...
    const nextBtn = modal.querySelector<HTMLButtonElement>('#next-level-btn');
  ...
  showHintMessage(message: string): void {
    ...
    const closeBtn = modal.querySelector<HTMLButtonElement>('#close-hint-btn');
    if (closeBtn) {
      closeBtn.addEventListener('click', () => modal.remove());
    }
  ...
  private updateBackgroundProgress(progress: number): void {
